...
# # Introduction to OpenCV - solutions for hw8 (cs35 spring 17)
...
def opencv_test1():
    ...
    raw_image = cv2.imread('monalisa.jpg',cv2.IMREAD_GRAYSCALE)

    # convert an OpenCV image (BGR) to an "ordinary" image (RGB)
    #image = cv2.cvtColor(raw_image, cv2.COLOR_BGR2RGB)

    plt.imshow(raw_image,cmap='seismic')

    plt.xticks([])
    plt.yticks([])  # to hide axis labels
    plt.show()
...
def opencv_test5():
    ...
    ax[0,1].imshow(image_b, cmap='Blues'); ax[0,1].set_title("Blue")
    ax[1,0].imshow(image_r, cmap='Reds'); ax[1,0].set_title("Red")
    ax[1,1].imshow(image_g, cmap='Greens'); ax[1,1].set_title("Green")
    for row in range(2):
        for col in range(2):
            ax[row,col].axis('off')
    plt.show(fig)
...
def opencv_test6():
    ...
    image_gt142_indices = gray_image > 200
    ...
    plt.title("Thresholded at gray level == 200")
    input("Hit return to continue...")
...
def new_filter( image ):
    """ an example of a pixel-by-pixel filter
        input: an r, g, b image
        output: an image with each color channel emphasized
    """
    new_image = image.copy()
    num_rows, num_cols, num_chans = new_image.shape
    for row in range(num_rows):
        for col in range(num_cols):
            r, g, b = image[row,col]
            new_image[row,col] = [r*3 , g*3, b*3 ]
    return new_image

def l_filter(image):
    image_r, image_g, image_b = cv2.split(image)
    hn = 255.0/2
    r_indices_h = image_r > hn
    r_indices_l = image_r < hn
    g_indices_h = image_g > hn
    g_indices_l = image_g < hn
    b_indices_h = image_b > hn
    b_indices_l = image_b < hn
    image_r[r_indices_h] = 255
    image_r[r_indices_l] = 0
    image_g[g_indices_h] = 255
    image_g[g_indices_l] = 0
    image_b[b_indices_h] = 255
    image_b[b_indices_l] = 0
    new_image = cv2.merge([image_r,image_g,image_b])
    return new_image
...
def two_image_filter( image1, image2 ):
    ...
    # image1 and image2 are resized to be the same size
    image1_resize = cv2.resize(image1, dsize=(min(widths),min(heights)), interpolation=cv2.INTER_AREA)
    image2_resize = cv2.resize(image2, dsize=(min(widths),min(heights)), interpolation=cv2.INTER_AREA)

    new_image = image1_resize.copy()
    num_rows, num_cols, num_chans = new_image.shape
    for row in range(num_rows):
        for col in range(num_cols):
            r1, g1, b1 = image1_resize[row,col]
            r2, g2, b2 = image2_resize[row,col]
            new_image[row,col] = [r1 , 0, b2*2 ]
...
def try_filter( image ):
    """ try out a filter! """
    from matplotlib import pyplot as plt
    new_image = l_filter( image )
    plt.imshow(new_image)
    plt.show()

def try_two_image_filter(image1,image2):
    from matplotlib import pyplot as plt
